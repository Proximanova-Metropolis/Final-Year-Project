...
import { AnalyzedResult } from 'src/app/models/models';
...
export class CanvasComponent implements OnInit, OnChanges {
  ...
  constructor(private analyticsrv:AnalyticsService, public modalController: ModalController, public toastController: ToastController) {
  ...
  async select(concentricCircleDatum: ConcentricCircleDatum) {
    ...
    //console.log('x', concentricCircleDatum);
    
    this.call_Analyse(concentricCircleDatum.pivot.data.content);
    this.redrawConcentricCircles();
    await this.zoomTo(concentricCircleDatum.id);
  }
 

  call_Analyse(comment_text:string){
    console.log('Comment :',comment_text);
    this.analyticsrv.AnalyzeComment(comment_text).subscribe(async data => {    
      let obj = <AnalyzedResult>data;

      this.analyticsrv.ReturnedData=obj;

      console.log(obj.senticnet1);
      const sentimentModal = await this.modalController.create({
        component: SentimentModalComponent,
        // message: 'senticnet: ' + obj.senticnet1  + '; sentiwordnet: ' + obj.sentiwordnet1 + '; vader: '+ obj.vader1,
        componentProps: {
          'sn': obj.senticnet1,
          'swn': obj.sentiwordnet1,
          'vd': obj.vader1
        }
      });
      sentimentModal.present();
    }) 
    }
