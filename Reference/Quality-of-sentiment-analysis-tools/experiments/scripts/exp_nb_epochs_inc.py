...
def main(args):
    ...
    for file in onlyfiles:
            ...
            nb_epochs=df["nb_epochs"].values
            #accs=df["acc"].values
            #print(accs)
            incs=df["inc"].values
            ax = fig.add_subplot (1 , 1 , 1)
            sns.lineplot (nb_epochs,incs  , ax=ax , markersize=5,marker=markers[i], linewidth=1,markerfacecolor='white', markeredgecolor='black',markeredgewidth=0.5)
            ax.spines['right'].set_visible (False)
            ax.spines['top'].set_visible (False)
            plt.xticks ([50,100,150,200,300],["5e","1e2","1.5e2","2e2","3e2"],rotation=45,fontsize=8)
            plt.yticks (rotation=45,fontsize=8)
            #ax = fig.add_subplot (3 , 8 , 2)
            #sns.lineplot (nb_epochs,accs  , ax=ax , markersize=5,marker=markers[i], linewidth=1,markerfacecolor='white',markeredgecolor='black',markeredgewidth=0.5)
            #ax.lines[i].set_linestyle (shape[i])
            #ax.spines['right'].set_visible (False)
            #ax.spines['top'].set_visible (False)

            #plt.xticks ([50,100,150,200,300],["50","100","150","200","3000"], rotation=45)
            #plt.yticks (rotation=45,fontsize=8)
    ...
    #ax.set_xlabel ("nb_epochs" , fontsize=8)
    #ax.set_title("Inc mean and nb_epochs",fontsize=10)
    #ax = fig.add_subplot (3 , 8 , 2)
    ax.set_ylabel ("Inconsistency_mean" , fontsize=8)
    ax.set_xlabel ("nb_epochs" , fontsize=8)
    #plt.figlegend (datasets , loc='center left' , ncol=3 , labelspacing=0., prop={'size': 7})
    ax.set_title("Inc & nb_epochs",fontsize=8)
