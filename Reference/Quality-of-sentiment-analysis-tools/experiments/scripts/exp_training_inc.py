...
def main(args):
    ...
    for file in onlyfiles:
        ...
        dfi_inc.loc[i] = df["inc"].to_numpy()
        #dfi_acc[datasets[i]] = df["acc"]
        i = i+1
    ...
    bars = dfi_inc.plot (kind='bar' , ax=ax , width=0.5 , colors=["#660000" , "#ff6666", "#ffb2b2"] , linewidth=0.5 ,
    ...
    #ax.set_title ("Training dataset  and accuracy" ,fontsize=8)
    #bars = dfi_acc.plot (kind='bar' , ax=ax , width=0.5 , colors=["#660000" , "#ff6666" , "#ffb2b2"] , linewidth=0.5 ,edgecolor="black" , alpha=0.8)
    #plt.legend ( prop={'size': 5})
    #plt.xticks ([0 , 1 , 2 , 3] , datasets , rotation=20, fontsize=6)
    mean_1=dfi_inc.iloc[0].mean ()
    mean_2 = dfi_inc.iloc[1].mean ()
    mean_3 = dfi_inc.iloc[2].mean ()
    print("mean_1",mean_1)
    print ("mean_2" , mean_2)
    print ("mean_3" , mean_3)
