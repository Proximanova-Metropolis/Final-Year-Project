...
def build(
  ...
  Args:
    ...
    discoveryzerviceUrl: string, a URI Template that points to the location of
    ...
    static_discovery: Boolean, whether or not to use the static discovery docs
      ...
      on the value of `discoveryzerviceUrl`. `static_discovery` will default to
      `True` when `discoveryzerviceUrl` is also not provided, otherwise it will
      default to `False`.
  ...
  """
    ...
    # `discoveryzerviceUrl`. `static_discovery` will default to `True` when
    # `discoveryzerviceUrl` is also not provided, otherwise it will default to
    ...
    if static_discovery is None:
        if discoveryzerviceUrl is None:
    ...
    for discovery_url in _discovery_service_uri_options(discoveryzerviceUrl, version):
...
def _discovery_service_uri_options(discoveryzerviceUrl, version):
    """
  ...
  Returns:
    ...
    if discoveryzerviceUrl is not None:
        return [discoveryzerviceUrl]
    if version is None:
