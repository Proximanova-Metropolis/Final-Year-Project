...
def main(args):
    ...
    for file in onlyfiles:
            ...
            sizes=df["dropout"].values
            accs=df["acc"].values
            #print(accs)
            #incs=df["inc"].values

            ax = fig.add_subplot (1 ,1 , 1)
            #sns.lineplot (sizes,incs  , ax=ax , markersize=6,marker=markers[i],linewidth=1,markerfacecolor='white', markeredgecolor='black',markeredgewidth=0.5)
    ...
    #ax.set_title("Inc and batch size",fontsize=8)
    #ax = fig.add_subplot (1 , 2 , 2)
    ax.set_ylabel ("Accuracy" , fontsize=8)
    ax.set_xlabel ("dropout p" , fontsize=8)
    #legend=plt.figlegend (datasets , loc='lower left' , ncol=3 , labelspacing=0.1, prop={'size':10})




    ax.set_title("Acc & dropout p",fontsize=8)
